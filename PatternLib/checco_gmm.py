...
from PatternLib.pipeline import Faucet
...
def gmm_em(X, gmm, threshold=10**-6, psi=0.01, model="FCG"):
    ll_prev = None
...
class GMM_checco(Faucet):
    def __init__(self, n_components=4, alpha=0.1, psi=0.01, model="FCG"):
    ...
    def fit(self, D, L):
        ...
        for c in self.classes:
            gmm_c, _, _ = gmm_lbg(D[:, L == c], self.n_components, alpha=self.alpha, psi=self.psi, model=self.model)
            self.gmm.append(gmm_c)

    def predict(self, x, return_prob=False):
        log_l = np.array([logpdf_gmm(x.T, self.gmm[g]) for g in range(len(self.gmm))])
        if return_prob:
            return np.argmax(log_l, axis=0), log_l[1, :] - log_l[0, :]
        return np.argmax(log_l, axis=0)
    ...
    def __str__(self):
        return f"GMM_checco(alpha: {self.alpha}, n: {self.n_components}, model: {self.model})"
